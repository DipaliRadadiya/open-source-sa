...
function AlertDialogFooter({
  ...
  return (
    <div
      ...
      className={cn(
        // Matches FormModal's footer at every width — a right-aligned row that
        // wraps. It used to be flex-col-reverse below sm, which stacked the two
        // buttons full-width AND put the destructive one on top, so a delete
        // confirmation looked nothing like the form dialog next to it.
        "-mx-4 -mb-4 flex flex-wrap items-center justify-end gap-2 rounded-b-xl border-t bg-muted/50 p-4 group-data-[size=sm]/alert-dialog-content:grid group-data-[size=sm]/alert-dialog-content:grid-cols-2",
        className
      )}
      {...props} />
  );
