...
return [
    ...
    */
    ...
    'help' => [
        'compose' => 'Необязательно. Вставьте свой файл compose — поддерживается всё, что поддерживает Compose: несколько служб, именованные тома, healthcheck. Оставьте пустым, и панель составит файл из полей выше. Порты должны публиковаться на 127.0.0.1, а bind-монтирования оставаться внутри каталога этого приложения; остальное отклоняется с указанием причины.',
        ...
        'docker_network' => 'Подключите контейнер к сети Docker, чтобы он и другие контейнеры в этой сети могли обращаться друг к другу по имени. Оставьте пустым для стандартного bridge Docker, где это невозможно. Сети создаются на странице Docker.',
