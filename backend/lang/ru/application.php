...
return [
    ...
    'unavailable' => [
        'stack' => 'Этот сервер запускает только контейнеры, поэтому не размещает приложения такого типа.',
    ...
    */
    ...
    'help' => [
        'image' => 'Образ для запуска с явным тегом — `nginx:1.27-alpine`. Имя без тега берёт `latest`, из-за чего развёртывание невоспроизводимо, а откат лишён смысла.',
        'container_port' => 'Порт, который ваше приложение слушает внутри контейнера. Порт на самом сервере назначает панель и направляет на него nginx.',
