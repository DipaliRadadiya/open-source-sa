...
return [
    ...
    'unavailable' => [
        'test_domain' => 'Единственные домены этого сайта — временные тестовые домены (:domains). Let\'s Encrypt не может выпустить для них сертификат, поскольку они делят один недельный лимит со всеми остальными пользователями этой службы. Самоподписанный сертификат зашифрует сайт прямо сейчас.',
