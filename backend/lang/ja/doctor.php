...
return [
    ...
    'fixes' => [
        'docker_missing' => 'Docker がインストールされていません。コンテナを実行する場合は「サービス」ページからインストールしてください。このサーバーの他の機能には必要ありません。',
        'docker_down' => 'Docker はインストールされていますが、デーモンが応答しません。`sudo systemctl status docker` で原因を確認し、`sudo systemctl start docker` を実行してください。それまでコンテナは起動できません。',
        'docker_denied' => 'デーモンは稼働していますが、パネルからのアクセスを拒否しました。`sudo php artisan panel:sudoers` を実行して sudo 権限を更新してください。回避策としてサイトユーザーを `docker` グループに追加しないでください。このグループへの所属はこのサーバーの root 権限と同等です。',
        'docker_exposure' => 'あるコンテナがポートをすべてのアドレスに公開しています。Docker は ufw より前に独自のファイアウォール規則を書き込むため、ファイアウォール画面で閉じていると表示されていても、そのポートはインターネットから到達可能です。127.0.0.1 に公開し直し、nginx にプロキシさせてください。パネルが作成するコンテナはそうなっています。',
