...
return [
    'checks' => [
        ...
        'home_access' => 'Acceso a la carpeta personal',
    ...
    'fixes' => [
        ...
        'home_open' => 'La carpeta personal de un usuario del sistema está abierta a todas las demás cuentas locales, así que cualquier archivo que sus sitios hayan escrito con permisos normales —sesiones, archivos .env, bases de datos— puede leerlo el usuario de cualquier otro sitio. Ejecuta `php artisan sites:resync`: da acceso al servidor web mediante el grupo del usuario y después cierra la carpeta. Si un usuario queda abierto porque uno de sus sitios PHP usa el pool compartido, aísla primero ese sitio desde su pantalla de PHP.',
        ...
        'php_interpreter_missing' => 'Un sitio está configurado con una versión de PHP que no está instalada en este servidor. OpenLiteSpeed indica el binario lsphp por su ruta y no lo comprueba al probar la configuración, así que el sitio aparece como Activo y responde 503 a cada petición. Instala esa versión de PHP desde la pantalla de PHP, o cambia el sitio a una versión instalada.',
        ...
        'privilege_outdated' => 'El permiso sudo de este servidor es anterior al panel que está ejecutando: los comandos indicados arriba son los que esta versión intentará ejecutar y sudo rechazará. Fallan con «a password is required» en funciones que parecen configuradas. Ejecute `sudo php artisan panel:sudoers` en el servidor para reescribir /etc/sudoers.d/ a partir de la lista del propio panel; valida el archivo con visudo antes de instalarlo.',
        ...
        'web_server_untestable' => 'El panel no tiene permiso para ejecutar la prueba de configuración del servidor web, así que no se pudo comprobar. Ejecuta `sudo php artisan panel:sudoers` en el servidor para actualizar el permiso de sudo; consulta la comprobación de privilegios para ver la causa.',
