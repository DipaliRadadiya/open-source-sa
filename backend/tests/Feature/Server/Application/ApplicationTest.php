...
describe('which git service the code came from', function () {
    /*
     * `site_type` is `git` for GitHub, GitLab and Bitbucket alike — one
     * GitSiteType covers all three, because the deploy recipe is identical and
     * only the account's API differs. So nothing on the application said where
     * the code came from, and a client wanting to show the provider had to
     * fetch every git account and join by hand.
     *
     * Resolved once at create, from the same data that decides the repository,
     * so the column cannot disagree with the URL beside it.
     */
    it('takes the provider from the connected account', function () {
        capableServer();
        ...
        $this->withHeaders(appHeaders())
            ->postJson('/api/applications', gitPayload())
            ->assertCreated()
            ->assertJsonPath('application.git_provider', 'github');

        expect(Application::first()->git_provider)->toBe('github');
    });

    it('reads the provider off a public repository url', function () {
        capableServer();
        ...
        $this->withHeaders(appHeaders())->postJson('/api/applications', [
            ...
            'branch' => 'main',
            'rendering_type' => 'php',
        ])->assertCreated()->assertJsonPath('application.git_provider', 'github');
    });

    it('says nothing rather than guessing for a self-hosted host', function () {
        // A self-hosted GitLab, a Gitea, a Bitbucket Server: the host names no
        // provider, and a wrong badge is worse than an absent one. This is the
        // guard — inferring "gitlab" from a hostname containing "git" is
        ...
        $this->withHeaders(appHeaders())->postJson('/api/applications', [
            ...
            'system_user_id' => $this->su->id,
            'git_source' => 'public_url',
            'repository_url' => 'https://git.acme.com/team/app.git',
            'branch' => 'main',
            'rendering_type' => 'php',
        ])->assertCreated()->assertJsonPath('application.git_provider', null);
    });

    it('carries the provider in the list as well as the detail', function () {
        capableServer();
        ...
        $this->withHeaders(appHeaders())->postJson('/api/applications', gitPayload())->assertCreated();

        $listed = collect($this->withHeaders(appHeaders())->getJson('/api/applications')->json('applications'));

        expect($listed->first()['git_provider'])->toBe('github');
    });
});
