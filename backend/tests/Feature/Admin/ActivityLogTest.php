...
it('returns the known distinct types and actions for filter dropdowns', function () {
    ...
    $response->assertOk()
        ->assertJsonPath('types', ['application', 'cronjob', 'database', 'disk_cleaner', 'fail2ban', 'firewall', 'git_account', 'log', 'permission', 'php', 'role', 'server', 'service', 'setting', 'system_user', 'user'])
        ->assertJsonCount(60, 'actions.all'); // all distinct verbs (deduped across types)
