...
it('returns the known distinct types and actions for filter dropdowns', function () {
    ...
    $response->assertOk()
        ...
        ->assertJsonCount(34, 'actions.all'); // all distinct verbs (deduped across types)
    // `all` = every verb; per-type keys are scoped to that type's verbs.
    expect($response->json('actions.all'))->toContain('registered', 'created', 'impersonation_started', 'ssh_key_added', 'sudo_enabled', 'shell_changed', 'ssh_enabled', 'downloaded', 'cleaned', 'schedule_updated');
