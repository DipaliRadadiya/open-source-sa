...
it('returns the known distinct types and actions for filter dropdowns', function () {
    ...
    $response->assertOk()
        ->assertJsonPath('types', ['application', 'cronjob', 'database', 'disk_cleaner', 'firewall', 'git_account', 'log', 'permission', 'role', 'service', 'setting', 'system_user', 'user'])
        ->assertJsonCount(52, 'actions.all'); // all distinct verbs (deduped across types)
