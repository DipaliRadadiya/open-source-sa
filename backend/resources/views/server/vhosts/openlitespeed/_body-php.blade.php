...
{{--
    The file that DESCRIBES a site: where its files are, where its logs go, what
    handles PHP. Included by `<name>.conf`, which DECLARES it — listeners,
    vhDomain, rewrite rules and TLS all live there now, because that is the half
    OpenLiteSpeed needs in order to route a request to this one at all.

    Splitting them is the old panel's arrangement and it is worth keeping for a
    reason beyond parity: the declaration is small enough to read, and this file
    can be regenerated without touching what the server matched on.
--}}
...
{{-- The old panel's four includes, in its order. Each is load-bearing:

     - `php.conf` — this site's PHP settings, its own file so a limits change
       rewrites that and nothing else.
     - the site's own snippets — where a customer puts OpenLiteSpeed directives,
       and where the old panel generates WordPress permalink rules and its bot
       blocker. A vhost rewritten without this disables both silently.
     - the ACME alias — how the old panel serves challenges. Harmless here,
       where a `context` block above already does it, and kept so a migrated
       server's file matches what it had.

     A glob that matches nothing is fine; the old panel has shipped these on
     servers where the directories are empty for years. --}}
include {{ $phpConfPath }}
