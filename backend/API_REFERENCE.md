...
- **On OpenLiteSpeed only 5 of the 13 types are available**: `wordpress`, `phpmyadmin`, `php`, `git`, `static`. The other 8 marketplace installers are greyed with a reason while OLS support is unproven on real hardware — this list widens as each is confirmed. nginx and Apache are unrestricted.
